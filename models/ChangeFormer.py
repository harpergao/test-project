...
from models.help_funcs import SpatiotemporalAttentionFull
import torch.nn.functional as F
...
class EncoderTransformer(nn.Module):
    def __init__(self, img_size=256, patch_size=16, in_chans=3, num_classes=2, embed_dims=[64, 128, 256, 512],
        ...
        # dpr:是一个列表，包含每个 block 的 Drop Path 概率（随机深度丢弃的概率）
        # 这里使用了 torch.linspace 来生成一个从 0 到 drop_path_rate 的线性空间，长度为sum(depths)所有深度之和
        # 这意味着每个 block 的 Drop Path 概率会随着深度的增加而逐渐增大
        # x.item()将 torch.Tensor 类型的值转换为 Python 的标量值
...
class OverlapPatchEmbed(nn.Module):
    ...
    # 避免使用PyTorch 默认初始化策略，以增强模型稳定性
    def _init_weights(self, m):
        
        # 对所有 nn.Linear 层的 weight 使用截断正态分布
        # trunc_normal_ 通常是从 timm 或自定义的工具函数中来的，确保不会采样到离均值太远的值， Transformer 中常见做法
        if isinstance(m, nn.Linear):
            trunc_normal_(m.weight, std=.02)
            if isinstance(m, nn.Linear) and m.bias is not None:
                nn.init.constant_(m.bias, 0)
        # 对所有 nn.LayerNorm 层的 bias 初始化为 0，weight 初始化为 1        
        elif isinstance(m, nn.LayerNorm):
            nn.init.constant_(m.bias, 0)
            nn.init.constant_(m.weight, 1.0)
        # 对所有 nn.Conv2d 层的权重进行正态分布初始化，标准差为 sqrt(2.0 / fan_out)，（ He 初始化，又称 Kaiming 初始化）
        elif isinstance(m, nn.Conv2d):
            fan_out = m.kernel_size[0] * m.kernel_size[1] * m.out_channels
            fan_out //= m.groups
            m.weight.data.normal_(0, math.sqrt(2.0 / fan_out))
    ...
    def forward(self, x):
        ...
        x = self.proj(x)
        _, _, H, W = x.shape
        # # flatten 从 (B, embed_dim, H_out, W_out) 转换为 (B, embed_dim, H_out * W_out) 
        # and transpose 形状变为 (B, H_out * W_out, embed_dim)
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0., sr_ratio=1):
        ...
        # 如果 维度dim 不能被 头数num_heads 整除，则抛出异常
        ...
        # 计算缩放因子，默认为提供的qk_scale 或者 1 / sqrt(head_dim)
        ...
        if sr_ratio > 1:
            # 如果 sr_ratio 大于 1，则使用卷积层进行下采样，缩小sr_ratio倍
    ...
    def forward(self, x, H, W):
        ...
        if self.sr_ratio > 1:
            ...
            # kv = (2, B, num_heads, N, C // num_heads)
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        # Identity 是一个空的模块，表示没有 Drop Path.当数据通过它时，它会原封不动地直接返回输入
        self.drop_path = DropPath(drop_path) if drop_path > 0. else nn.Identity()
        self.norm2 = norm_layer(dim)
        mlp_hidden_dim = int(dim * mlp_ratio)
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, act_layer=act_layer, drop=drop)
...
class DWConv(nn.Module):
    def __init__(self, dim=768):
        ...
        # stride=1 和 padding=1：保持特征图的空间尺寸不变
        # 当 groups 参数被设置为输入通道数 dim 时，PyTorch 会为每个输入通道分配一个独立的卷积核
        self.dwconv = nn.Conv2d(dim, dim, 3, 1, 1, bias=True, groups=dim)

    def forward(self, x, H, W):
        #进行x = x.transpose(1, 2).view(B, C, H, W)是为了将transformer的数据流变为CNN处理的数据流
        # x = x.flatten(2).transpose(1, 2)在变回去
        B, N, C = x.shape
        x = x.transpose(1, 2).view(B, C, H, W)
        x = self.dwconv(x)
        # flatten 将特征图从 (B, C, H, W) 转换为 (B, C, H*W)
        x = x.flatten(2).transpose(1, 2)
...
class DecoderTransformer_v4(nn.Module):
    """
    Transformer Decoder
    """
    def __init__(self, input_transform='multiple_select', in_index=[0, 1, 2, 3], align_corners=True, 
        ...
        super(DecoderTransformer_v4, self).__init__()
        #assert
        assert len(feature_strides) == len(in_channels)
        assert min(feature_strides) == feature_strides[0]
        ...
        self.STAttentionModule_c4 = SpatiotemporalAttentionFull(in_channels=c4_in_channels)
        self.STAttentionModule_c3 = SpatiotemporalAttentionFull(in_channels=c3_in_channels)
        self.STAttentionModule_c2 = SpatiotemporalAttentionFull(in_channels=c2_in_channels)
        self.STAttentionModule_c1 = SpatiotemporalAttentionFull(in_channels=c1_in_channels)
