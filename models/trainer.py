...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        if torch.cuda.is_available():
            print("CUDA is available. Using GPU:", args.gpu_ids)
        else:
    ...
    def _update_checkpoints(self):
        """
        更新检查点,
        - 每个 epoch 保存最新的模型检查点(last_ckpt.pt)
        - 如果当前验证准确率超过历史最佳值，则更新最佳模型检查点(best_ckpt.pt)        """
        # save current model
    ...
    # def _backward_G(self):
    #     gt = self.batch['L'].to(self.device).float()
    #     print("GT shape:", gt.shape, "NaN:", torch.isnan(gt).any())

    #     if self.multi_scale_train == "True":
    #         temp_loss = 0.0
    #         for i, pred in enumerate(self.G_pred):
    #             print(f"Pred[{i}] shape:", pred.shape, "NaN:", torch.isnan(pred).any())
    #             try:
    #                 if pred.size(2) != gt.size(2):
    #                     loss = self.weights[i]*self._pxl_loss(pred, F.interpolate(gt, size=pred.size(2), mode="nearest"))
    #                 else:
    #                     loss = self.weights[i]*self._pxl_loss(pred, gt)

    #                 print(f"Loss[{i}]:", loss.item(), "NaN:", torch.isnan(loss).any())
    #                 temp_loss = temp_loss + loss
    #             except Exception as e:
    #                 print(f"Error at pred[{i}]:", e)
    #                 raise
    #         self.G_loss = temp_loss
    #     else:
    #         self.G_loss = self._pxl_loss(self.G_pred[-1], gt)
    #         print("G_loss:", self.G_loss.item(), "NaN:", torch.isnan(self.G_loss))
